...
object WebCrawler extends App {
    ...
    def wget(url: URL)(implicit ec: ExecutionContext): Future[Seq[URL]] = {
        ...
        // 16 points.
        def getURLs(ns: Node): Seq[Try[URL]] = for (n <- ns \\ "a"; nl =  n \@ "href") yield validateURL(createURL(Some(url), nl.toString())) // TO BE IMPLEMENTED
        ...
        // 9 points.
        for(l <- getURLContent(url); ls <- MonadOps.asFuture(getLinks(l))) yield ls// TO BE IMPLEMENTED
    }
